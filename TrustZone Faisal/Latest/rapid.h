...
/*---------------------------------------------------------------
 * get_program_hash()
 *   • Maps /proc/self/exe read-only
 *   • Hashes the entire file in ONE call using EVP
 *   • Returns malloc'ed 65-byte string (64 hex chars + NUL)
 *   • Caller must free() the returned pointer.
 *   • Returns NULL on error.
 *--------------------------------------------------------------*/
 static inline char *get_program_hash(void)
 {
     ...
     unsigned char digest[SHA256_DIGEST_LENGTH];
     EVP_Digest(map, (size_t)st.st_size, digest, NULL, EVP_sha256(), NULL);
