...
int main(void) {
    ...
    char *initial_hash = get_program_hash();
    if(!initial_hash) {perror("sha256"); exit(EXIT_FAILURE);}
    printf("\nProgram digest: %s\n\n", initial_hash);
    free(initial_hash);
    //printf("Additional printf test\n\n");
    idle();
    ...
    while (1) {
        ...
        switch (cmd.choice) {
            ...
            case 3:
                ...
                spin_ninety();
                printf("spin_ninety hash: %s\n", hash_function(spin_ninety, SPIN_NINETY_LENGTH));
                printf("Current hash chain: %s\n", get_hash_chain_current());
                break;
            case 4: 
                PRINT_OPCODES(rest);
