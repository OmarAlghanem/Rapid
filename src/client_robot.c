...
void gripper_ninety() {
    Hash_chain(gripper_ninety, GRIPPER_NINETY_LENGTH);
    printf("Executing gripper_ninety() [Hash Updated]...\n");
    softPwmWrite(GRIPPER, 15); delay(1000);
    softPwmWrite(GRIPPER, 0); delay(500); // Shorter delay after stop maybe enough
    printf("gripper_ninety() finished.\n");
}
END_FUNCTION(gripper_ninety)

void gripper_oneeighty() {
    Hash_chain(gripper_oneeighty, GRIPPER_ONEEIGHTY_LENGTH);
    printf("Executing gripper_oneeighty() [Hash Updated]...\n");
    softPwmWrite(GRIPPER, 25); delay(1000);
    softPwmWrite(GRIPPER, 0); delay(500); // Shorter delay after stop maybe enough
    printf("gripper_oneeighty() finished.\n");
}
END_FUNCTION(gripper_oneeighty)

void spin_ninety4() {
    Hash_chain(spin_ninety4, SPIN_NINETY_LENGTH4);
    printf("Executing spin_ninety4() [Hash Updated]...\n");
    softPwmWrite(FOURTH_SERVO, 15); delay(1000);
    softPwmWrite(FOURTH_SERVO, 0); delay(500); // Shorter delay after stop maybe enough
    printf("spin_ninety4() finished.\n");
}
END_FUNCTION(spin_ninety4)

void spin_oneeighty4() {
    Hash_chain(spin_oneeighty4, SPIN_ONEEIGHTY_LENGTH4);
    printf("Executing spin_oneeighty4() [Hash Updated]...\n");
    softPwmWrite(FOURTH_SERVO, 25); delay(1000);
    softPwmWrite(FOURTH_SERVO, 0); delay(500); // Shorter delay after stop maybe enough
    printf("spin_ninety4() finished.\n");
}
END_FUNCTION(spin_oneeighty4)

void rest4() {
    Hash_chain(rest, REST_LENGTH4);
    printf("Executing rest4() [Hash Updated]...\n");
    softPwmWrite(FOURTH_SERVO, 5); delay(1000);
    softPwmWrite(FOURTH_SERVO, 0); delay(500);
    printf("rest4() finished.\n");
}
END_FUNCTION(rest4)

void spin_ninety3() {
    Hash_chain(spin_ninety3, SPIN_NINETY_LENGTH3);
    printf("Executing spin_ninety3() [Hash Updated]...\n");
    softPwmWrite(THIRD_SERVO, 15); delay(1000);
    softPwmWrite(THIRD_SERVO, 0); delay(500); // Shorter delay after stop maybe enough
    printf("spin_ninety3() finished.\n");
}
END_FUNCTION(spin_ninety3)

void spin_oneeighty3() {
    Hash_chain(spin_oneeighty3, SPIN_ONEEIGHTY_LENGTH3);
    printf("Executing spin_oneeighty3() [Hash Updated]...\n");
    softPwmWrite(THIRD_SERVO, 25); delay(1000);
    softPwmWrite(THIRD_SERVO, 0); delay(500); // Shorter delay after stop maybe enough
    printf("spin_ninety3() finished.\n");
}
END_FUNCTION(spin_oneeighty3)

void rest3() {
    Hash_chain(rest, REST_LENGTH3);
    printf("Executing rest3() [Hash Updated]...\n");
    softPwmWrite(THIRD_SERVO, 5); delay(1000);
    softPwmWrite(THIRD_SERVO, 0); delay(500);
    printf("rest3() finished.\n");
}
END_FUNCTION(rest3)

void spin_ninety2() {
    Hash_chain(spin_ninety2, SPIN_NINETY_LENGTH2);
    printf("Executing spin_ninety2() [Hash Updated]...\n");
    softPwmWrite(SECOND_SERVO, 15); delay(1000);
    softPwmWrite(SECOND_SERVO, 0); delay(500); // Shorter delay after stop maybe enough
    printf("spin_ninety2() finished.\n");
}
END_FUNCTION(spin_ninety2)

void spin_oneeighty2() {
    Hash_chain(spin_oneeighty2, SPIN_ONEEIGHTY_LENGTH2);
    printf("Executing spin_ninety() [Hash Updated]...\n");
    softPwmWrite(SECOND_SERVO, 25); delay(1000);
    softPwmWrite(SECOND_SERVO, 0); delay(500); // Shorter delay after stop maybe enough
    printf("spin_oneeighty2() finished.\n");
}
END_FUNCTION(spin_oneeighty2)

void rest2() {
    Hash_chain(rest, REST_LENGTH2);
    printf("Executing rest2() [Hash Updated]...\n");
    softPwmWrite(SECOND_SERVO, 5); delay(1000);
    softPwmWrite(SECOND_SERVO, 0); delay(500);
    printf("rest2() finished.\n");
}
END_FUNCTION(rest2)
...
int comm_send_current_hash(const char *context_message) {
    ...
    char msg[BUFFER_SIZE]; // From client_robot.h
    ...
    if (bytes <= 0) {
        ...
        ERR_print_errors_fp(stderr);
        // *** Check for specific SSL errors if needed ***
        // if (err == SSL_ERROR_ZERO_RETURN) { printf("Peer closed connection cleanly.\n"); }
        // if (err == SSL_ERROR_SYSCALL) { perror("Underlying socket error during SSL_read"); }
        return -1;
    }
    buffer[bytes] = '\0';
