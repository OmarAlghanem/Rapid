...
static inline char *hash_function(void (*func)(), size_t length) {
    ...
    SHA256((unsigned char*)func, length, digest);
...
static inline unsigned char *Hash_chain(void (*func)(), size_t length) {
    if (!__hash_chain_initialized) {
        SHA256((unsigned char*)func, length, __hash_chain_state);
        __hash_chain_initialized = 1;
    } else {
        SHA256_CTX ctx;
        SHA256_Init(&ctx);
        SHA256_Update(&ctx, __hash_chain_state, SHA256_DIGEST_LENGTH);
        SHA256_Update(&ctx, (unsigned char*)func, length);
        SHA256_Final(__hash_chain_state, &ctx);
    }
